...
#  curl -sL https://raw.githubusercontent.com/nipegun/df-scripts/refs/heads/main/Reversing/00-SandBox-Crear.sh -o /tmp/sb.sh && chmod +x /tmp/sb.sh && sed -i 's-$HOME-/mnt/PartLocalBTRFS-g' /tmp/sb.sh  && sed -i 's-sudo--g' /tmp/sb.sh && /tmp/sb.sh  [CarpetaAMontar]
...
# Iniciar el sandbox con aislamiento y carpeta compartida
  ...
  echo "      curl -sL https://raw.githubusercontent.com/nipegun/dh-scripts/refs/heads/main/Recursos/SystemdSandbox/InSandBox-Debian-Preparar-Base.sh | ba"
  echo ""
  echo ""
  echo "    Y pega estos otros comandos para preparar el contenedor para cada caso específico:"
  echo ""
  echo "      Para reversing:"  
  echo "        curl -sL https://raw.githubusercontent.com/nipegun/df-scripts/refs/heads/main/Reversing/InSystemdContainer-Preparar-ParaReversing.sh | bash"
  echo ""
